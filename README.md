...
Готовые файлы находятся в [последнем релизе](https://github.com/cyberboy1999/pirate-cinema/releases/latest). Сборки 0.5.8 предназначены для 64-битных систем.

### Windows 10/11

`Pirate-Cinema-Setup-0.5.8.exe` — рекомендуемый автономный установщик со встроенными MPV, FFmpeg и TorrServer. Запустите файл, подтвердите запрос Windows UAC и следуйте мастеру.

`Pirate-Cinema-Web-Setup-0.5.8.exe` — небольшой веб-установщик. Во время установки ему нужен стабильный доступ к GitHub. Если GitHub доступен только через VPN, используйте автономный установщик.
...
```sh
sudo apt install ./Pirate-Cinema-0.5.8-linux-amd64.deb
```
...
```sh
sudo dnf install ./Pirate-Cinema-0.5.8-linux-x86_64.rpm
```

На системах с YUM используйте `sudo yum install ./Pirate-Cinema-0.5.8-linux-x86_64.rpm`. Файл `install-linux.sh` из релиза автоматически выбирает APT, DNF или YUM и создаёт ярлык для KDE, GNOME или XFCE.
...
curl -LO https://github.com/cyberboy1999/pirate-cinema/releases/download/v0.5.8/PKGBUILD
makepkg -si
```
...
Download version 0.5.8 from the [latest release](https://github.com/cyberboy1999/pirate-cinema/releases/latest). Packages target 64-bit systems.

#### Windows 10/11

Use `Pirate-Cinema-Setup-0.5.8.exe` for the recommended offline installation. It includes MPV, FFmpeg and TorrServer. Run it, accept the Windows UAC prompt and follow the installer.

`Pirate-Cinema-Web-Setup-0.5.8.exe` is smaller but downloads the application package from GitHub. Use the offline installer when GitHub requires a VPN or the connection is unreliable.
