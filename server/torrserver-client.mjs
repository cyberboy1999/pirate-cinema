export class TorrServerClient {
  ...
  async configureTorznab({host,key}){const sets=await this.request("/settings",{method:"POST",body:JSON.stringify({action:"get"})});return this.request("/settings",{method:"POST",body:JSON.stringify({action:"set",sets:{...sets,EnableTorznabSearch:Boolean(host&&key),TorznabUrls:host&&key?[{Host:host,Key:key,Name:"Prowlarr / Jackett"}]:[]}})});}
