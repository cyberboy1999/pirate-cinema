export class TorrServerClient {
  ...
  async searchTorznab(query){const result=await this.request(`/torznab/search/?query=${encodeURIComponent(query)}`,{timeoutMs:20000});return Array.isArray(result)?result:[]}
  async configureTorznab({host,key}){const sets=await this.request("/settings",{method:"POST",body:JSON.stringify({action:"get"})});return this.request("/settings",{method:"POST",body:JSON.stringify({action:"set",sets:{...sets,EnableTorznabSearch:Boolean(host&&key),TorznabUrls:host&&key?[{Host:host,Key:key,Name:"Prowlarr / Jackett",CatType:"all"}]:[]}})});}
  ...
  async addTorrent({magnet,title,poster=null,category=""}){const normalized=normalizeTorrentLink(magnet);const existing=normalized.hash?(await this.listTorrents()).find(item=>String(item.hash??item.Hash??item.info_hash??"").toLowerCase()===normalized.hash):null;if(existing)return {result:existing,hash:normalized.hash,magnet:normalized.link,alreadyExists:true};const result=await this.request("/torrents",{method:"POST",body:JSON.stringify({action:"add",link:normalized.link,title:String(title??"").trim(),poster:poster||undefined,category:String(category??""),save_to_db:true})});const responseHash=normalizeInfoHash(result?.hash??result?.Hash??result?.info_hash??result?.torrent_hash);return {result,hash:responseHash??normalized.hash,magnet:normalized.link,alreadyExists:false}}
...
function safeTorrentUrl(input){try{const url=new URL(String(input??""));return ["http:","https:"].includes(url.protocol)?url.toString():""}catch{return ""}}
function normalizeTorrentLink(input){try{const magnet=normalizeMagnetLink(input);return {link:magnet.magnet,hash:magnet.hash}}catch{const link=safeTorrentUrl(input);if(!link)throw new Error("Нужна magnet-ссылка или HTTP(S)-ссылка на torrent-файл");return {link,hash:null}}}
