...
test("manual metadata query survives sync and drives enrichment",async()=>{
  ...
  const sync=createLibrarySync({db,state:{},cachePoster:async()=>"manual.jpg",
    client:{echo:async()=>"MatriX",listTorrents:async()=>[{hash,title:"Bad.Release.Name.2024.2160p"}],listViewed:async()=>[]},
  ...
  try{
    await sync.synchronize();db.setMetadataQuery(hash,"Dune");await sync.enrich(hash,true);await sync.synchronize(true);
    assert.deepEqual(queries,["Bad.Release.Name.2024.2160p","Dune 2024","Dune 2024"]);
    assert.equal(sync.item(hash).title,"Dune");assert.equal(sync.item(hash).overview,"Description");assert.equal(sync.item(hash).posterPath,"/api/posters/manual.jpg");
  }finally{db.close()}
});
